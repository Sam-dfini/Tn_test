        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={prepareList(actors).map(a => ({ name: a.name, score: natoScores[a.id], color: a.color }))}
                layout="vertical"
              ...
              >
                ...
                <Bar dataKey="score" radius={[0, 4, 4, 0]} barSize={20}>
                  {prepareList(actors).map((entry, index) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {prepareList(actors).map((actor, idx) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {prepareList(alignmentData.filter(d => d.weight >= 0.9)).map((dim, index) => {
        ...
        <div className="lg:col-span-4 space-y-6 md:space-y-8">
          <div className="glass p-4 md:p-8 rounded-xl md:rounded-3xl border border-intel-border space-y-6 md:space-y-8 relative overflow-hidden">
            ...
            <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2 custom-scrollbar">
              {prepareList(strategicRisks).map(risk => (
                <motion.div 
                  key={risk.id} 
                ...
                >
                  ...
                  <div className="flex items-center justify-between pl-2 border-t border-white/5 pt-4">
                    <div className="flex -space-x-2">
                      {risk.actors.map((a, aIdx) => {
                        const actor = actors.find(act => act.name === a || act.id === a.toLowerCase());
                        return (
                          <div key={generateStableKey(a, aIdx, 'risk-actor')} className="w-8 h-8 rounded-full bg-intel-bg border border-white/10 flex items-center justify-center shadow-lg hover:z-10 transition-all group/actor" title={a}>
          ...
          {prepareList([
            ...
            <div key={generateStableKey(p, i, 'regional-power')} className={`p-5 rounded-xl border space-y-4 ${p.risk === 'CRITICAL' ? 'border-intel-red/30 bg-intel-red/5' : p.risk === 'HIGH' ? 'border-intel-orange/20' : 'border-intel-border'}`}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {prepareList(Object.entries(p.vectors)).map(([key, val]: [string, any], vIdx: number) => (
                  <div key={generateStableKey({ key, val }, vIdx, 'vector')} className="space-y-1">
    ...
    >
      ...
      <div className="glass p-4 md:p-8 rounded-xl md:rounded-3xl border border-intel-border relative overflow-hidden z-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          ) : prepareList(data.geopolitical.diplomatic_relations as any[]).map((rel: any, idx: number) => (
