...
export const SocialMonitor: React.FC = () => {
  ...
  const fetchSocialData = async () => {
    setLoading(true);
    try {
      const prompt = `Generate a tactical social media monitoring report for Tunisia. 
...
{
  "hashtags": [
    { "tag": "#tag", "count": "123K", "trend": "up" | "down" | "stable" }
  ],
  "posts": [
    { 
      "platform": "WHATSAPP" | "TELEGRAM" | "TWITTER" | "FACEBOOK" | "DISCORD",
  ...
  return (
    <div className="glass rounded-lg border border-intel-border flex flex-col h-[450px] overflow-hidden">
      {/* Header */}
      <div className="bg-black/40 border-b border-intel-border p-2 flex items-center gap-2">
        <div className="flex items-center space-x-1.5 min-w-0 flex-1">
          <Share2 className="w-3.5 h-3.5 text-intel-cyan shrink-0" />
          <h3 className="text-[10px] font-mono text-intel-cyan uppercase font-bold tracking-tight truncate">Social Monitor</h3>
        </div>
        <div className="flex items-center space-x-1.5 shrink-0">
          ...
          <div className="flex items-center gap-1">
            <button 
              ...
              className={`flex items-center space-x-1 px-1.5 py-1 rounded-lg transition-all duration-300 border ${
                activeTab === 'HASHTAGS' 
                  ? 'bg-intel-cyan/10 border-intel-cyan text-intel-cyan shadow-[0_0_10px_rgba(0,242,255,0.1)]' 
                  : 'bg-white/5 border-intel-border text-slate-500 hover:border-white/20 hover:text-white'
              }`}
            >
              <Hash className={`w-2.5 h-2.5 ${activeTab === 'HASHTAGS' ? 'text-intel-cyan' : 'text-slate-500'}`} />
              <span className="text-[8px] font-mono font-bold uppercase tracking-tighter">Tags</span>
            </button>
            <button 
            ...
            >
              <MessageSquare className={`w-2.5 h-2.5 ${activeTab === 'SOCIAL' ? 'text-intel-cyan' : 'text-slate-500'}`} />
              <span className="text-[8px] font-mono font-bold uppercase tracking-tighter">Feed</span>
            </button>
          </div>
        </div>
      </div>
