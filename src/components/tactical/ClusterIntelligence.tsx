    ...
    {activeTab === 'PROFILE' && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 relative z-20">
        ...
        <div className="space-y-3">
          {prepareList(clusterDetails).map((cluster: any, idx: number) => {
            const ClusterIcon = cluster.icon;
            return (
            <motion.div
              key={generateStableKey(cluster, idx, 'cluster')}
    ...
    {activeTab === 'GEOGRAPHIC' && (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {prepareList(clusterGroups).map((g: any, i: number) => (
            <div key={generateStableKey(g, i, 'cluster-group')} className="glass rounded-xl border border-intel-border p-4 space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {prepareList(g.govs).map((gov: any, j: number) => (
                  <span key={generateStableKey(gov, j, 'gov-badge')} className="text-[8px] font-mono bg-white/5 px-1.5 py-0.5 rounded text-slate-400">{gov}</span>
                ))}
              </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={governorateClusterData} layout="vertical" margin={{ left: 70 }}>
                ...
                <Bar dataKey="composite" radius={[0,4,4,0]} name="Composite Score">
                  {prepareList(governorateClusterData).map((entry: any, i: number) => (
                    <Cell key={generateStableKey(entry, i, 'cell')} fill={entry.composite > 0.7 ? '#ef4444' : entry.composite > 0.55 ? '#f97316' : entry.composite > 0.45 ? '#8b5cf6' : '#10b981'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            <table className="w-full min-w-[500px]">
              <thead>
                <tr className="border-b border-white/10">
                  {prepareList(['Governorate', 'Sys Pressure', 'Mobilization', 'Regime Fragility', 'Spread Risk', 'Composite']).map((h: any, index: number) => (
        ...
        <div className="glass rounded-xl border border-intel-border p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-4 text-[9px] font-mono">
            {prepareList([
              ...
              <span key={generateStableKey(l, i, 'legend')} className="flex items-center gap-1.5">
