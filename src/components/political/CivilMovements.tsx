...
export const CivilMovements: React.FC = () => {
  ...
  const handleAIAnalysis = async (movement: any) => {
    setIsAnalyzing(movement.id);
    try {
      const prompt = `Perform a deep strategic analysis of the civil movement: ${movement.name}. 
      Type: ${movement.type}. 
      Risk Level: ${movement.riskLevel}. 
      Hotspots: ${movement.hotspots.join(', ')}.
      Demands: ${movement.demands.join(', ')}.
      Last Action: ${movement.lastAction}.
      Description: ${movement.description}.
      
      Focus on their potential for escalation, coordination with other groups, and their impact on the overall RRI (Rapid Risk Indicator) which is currently ${rriState.rri.toFixed(2)}.`;
      
      const response = await generateAnalystResponse(prompt, { rri: rriState.rri, pRev: rriState.p_rev });
      if (response) {
        setAiAnalysis(prev => ({ ...prev, [movement.id]: response }));
      }
    } catch (error) {
  ...
  const filteredMovements = useMemo(() => {
    return movementData.filter(m => {
      ...
      const matchesSearch = searchQuery === '' || 
        ...
        m.hotspots.some(h => h.toLowerCase().includes(searchQuery.toLowerCase())) ||
        m.demands.some(d => d.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 relative">
      ...
      <ModuleHeader 
        title="Civil Movements Intelligence"
        subtitle="Real-time monitoring of social friction, protest frequency, and civil society actors"
        icon={Users}
        nodeId="CIV-MOVE-19"
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative z-20">
        {stats.map((stat) => (
          <div key={`stat-${stat.label}`} className="intel-card p-6 rounded-2xl border border-intel-border flex flex-col justify-between">
              ...
              <div className="flex items-center space-x-2 overflow-x-auto scrollbar-hide">
                {['ALL', 'ECONOMIC', 'SOCIAL', 'INSTITUTIONAL'].map(t => (
                  <button 
                    ...
                    onClick={() => setFilter(t)}
                    className={cn(
                      "px-2 py-1 rounded text-[8px] font-mono uppercase tracking-widest transition-all whitespace-nowrap",
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {movement.hotspots.map((h) => (
                    <span key={`hotspot-${movement.id}-${h}`} className="px-2 py-0.5 bg-white/5 border border-white/10 rounded text-[9px] text-slate-400 flex items-center">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-3">
                          ...
                          <div className="space-y-1">
                            ...
                            <ul className="space-y-1">
                              {movement.demands.map((d) => (
                                <li key={`demand-${movement.id}-${d}`} className="text-[10px] text-slate-400 flex items-center">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="pt-4 border-t border-white/5 space-y-3">
                            <div className="flex items-center justify-between">
                              <h5 className="text-[10px] font-bold text-intel-cyan uppercase tracking-widest flex items-center">
                                <Sparkles className="w-3 h-3 mr-2 text-intel-cyan" />
                                AI Strategic Intelligence
                              </h5>
                              {!aiAnalysis[movement.id] && (
                                <button 
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    handleAIAnalysis(movement);
                                  }}
                                  disabled={isAnalyzing === movement.id}
                                  className="px-2 py-1 bg-intel-cyan/10 border border-intel-cyan/30 rounded-lg text-[8px] font-mono text-intel-cyan uppercase tracking-widest hover:bg-intel-cyan hover:text-black transition-all flex items-center space-x-1 disabled:opacity-50"
                                >
                                  {isAnalyzing === movement.id ? (
                                    <>
                            ...
                            {aiAnalysis[movement.id] ? (
                              <div className="p-3 bg-black/40 rounded-xl border border-intel-cyan/20 text-[10px] text-slate-300 leading-relaxed font-mono overflow-y-auto max-h-[200px] custom-scrollbar">
                                <div className="markdown-body">
                                  <Markdown>{aiAnalysis[movement.id]}</Markdown>
                                </div>
                              </div>
                            ) : (
                              <p className="text-[9px] text-slate-500 italic">No AI analysis generated for this movement yet.</p>
                            )}
                          </div>
                        </div>
                      </div>
