...
export const PartyDossier: React.FC = () => {
  ...
  const handleAIAnalysis = async (party: any) => {
    setIsAnalyzing(party.id);
    try {
      const prompt = `Perform a deep strategic analysis of the political party: ${party.name} (${party.arabicName}). 
      Current Leader: ${party.leader} (${party.leaderStatus}). 
      Ideology: ${party.ideology}. 
      Alignment: ${party.alignment}. 
      Status: ${party.status}.
      Recent Events: ${party.recentEvents.join(', ')}.
      
      Focus on their current organizational resilience, potential for mobilization in Q3 2026, and their impact on the overall RRI (Rapid Risk Indicator) which is currently ${rriState.rri.toFixed(2)}.`;
      
      const response = await generateAnalystResponse(prompt, { rri: rriState.rri, pRev: rriState.p_rev });
      if (response) {
        setAiAnalysis(prev => ({ ...prev, [party.id]: response }));
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 relative">
      ...
      <ModuleHeader 
        title="Political Party Intelligence"
        subtitle="Strategic dossiers on political actors, ideological alignments, and mobilization strength"
        icon={Users}
        nodeId="PARTY-NODE-21"
      />
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="pt-4 border-t border-white/5 space-y-4">
                          <div className="flex items-center justify-between">
                            <h4 className="text-[10px] font-bold text-intel-cyan uppercase tracking-widest flex items-center">
                              <Sparkles className="w-3 h-3 mr-2 text-intel-cyan" />
                              AI Strategic Intelligence
                            </h4>
                            {!aiAnalysis[party.id] && (
                              <button 
                                onClick={() => handleAIAnalysis(party)}
                                disabled={isAnalyzing === party.id}
                                className="px-3 py-1 bg-intel-cyan/10 border border-intel-cyan/30 rounded-lg text-[8px] font-mono text-intel-cyan uppercase tracking-widest hover:bg-intel-cyan hover:text-black transition-all flex items-center space-x-2 disabled:opacity-50"
                              >
                                {isAnalyzing === party.id ? (
                                  <>
                          ...
                          {aiAnalysis[party.id] ? (
                            <div className="p-4 bg-black/40 rounded-xl border border-intel-cyan/20 text-[10px] text-slate-300 leading-relaxed font-mono overflow-y-auto max-h-[300px] custom-scrollbar">
                              <div className="markdown-body">
                                <Markdown>{aiAnalysis[party.id]}</Markdown>
                              </div>
                            </div>
                          ) : (
                            <p className="text-[10px] text-slate-500 italic">No AI analysis generated for this dossier yet.</p>
                          )}
                        </div>
                      </div>
                      <div className="space-y-6">
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {party.internationalLinks.map((link, index) => (
                              <span key={`${link}-${index}`} className="flex items-center space-x-1 px-2 py-1 bg-white/5 border border-white/10 rounded-lg text-[9px] text-slate-400">
