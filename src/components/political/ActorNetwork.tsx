...
export const ActorNetwork: React.FC<{ context?: any }> = ({ context }) => {
  ...
  const handleScanActors = async () => {
    ...
    try {
      ...
      IMPORTANT: Your response must be a single, valid JSON array of objects. Do not include any introductory or concluding text.
      ...
      [
        {
          ...
          "name": "Full Name",
          "role": "Current Role",
          "alignment": "GOV" | "OPP" | "INTL" | "NEUTRAL",
          "influence": 0-100,
          "threat_level": "STATE" | "DETAINED" | "HIGH" | "MEDIUM" | "LOW" | "MONITOR" | "LEVERAGE",
          "status": "ACTIVE" | "DETAINED" | "INACTIVE",
          "description": "Brief analysis of their current standing and impact."
        }
      ]`;

      const response = await generateAnalystResponse(prompt, context, { responseMimeType: 'application/json' });
      if (!response) throw new Error('No response from AI');
      const newActors = parseAIJSON(response);
  ...
  return (
    <div className="flex flex-col h-full p-3 md:p-4 space-y-12 animate-in fade-in duration-500 relative">
      <BackgroundGrid />
      
      <ModuleHeader 
        title="Actor Network Intelligence"
        subtitle="Mapping influence, alignment, and threat levels across the Tunisian political-security landscape"
        icon={Network}
