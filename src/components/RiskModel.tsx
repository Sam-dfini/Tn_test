  ...
  const applyScenario = (scenarioId: string) => {
    ...
    setTimeout(() => {
      setVariables(prev => prev.map(v => {
        ...
        const impactValue = scenario.impact[v.id as keyof typeof scenario.impact] || 
                           scenario.impact[v.name.toLowerCase() as keyof typeof scenario.impact];
        
        if (impactValue !== undefined) {
          return { ...v, value: impactValue };
        }
        return v;
      }));
