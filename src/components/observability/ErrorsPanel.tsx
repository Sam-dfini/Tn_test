...
export const ErrorsPanel: React.FC<ErrorsPanelProps> = ({ errors }) => {
  return (
    <div className="bg-[#1a1a1a] border border-white/10 rounded-lg p-5 flex flex-col space-y-4 max-h-[400px]">
          ...
          errors.slice(-15).reverse().map((err, i) => (
            <div key={err.id || i} className="p-2 bg-red-500/5 border border-red-500/10 rounded text-[10px] font-mono group">
              <div className="flex justify-between items-start gap-2">
                ...
                <span className="text-white/20 whitespace-nowrap">{new Date(err.time).toLocaleTimeString()}</span>
              </div>
              {err.stack && (
