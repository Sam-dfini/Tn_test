...
import { Map } from './Map';
import { EconomicReality } from './EconomicReality';
...
export const CitizenEdition: React.FC<CitizenEditionProps> = ({ governorates, events: propEvents, rri, pRev, onOpenAI, onOpenPipeline, onGoHome, onOpenReport, data }) => {
  const { rriState } = usePipeline();
  const { events: contextEvents, fetchNow } = useRSS();
  
  // Use context events if available, otherwise fallback to prop events
  ...
  // Generate default SME digital report
  const quickReport = useMemo(() => {
    try {
      const govAssessment = assessGovernmentAgent(rriState, data, {});
      return generateInvestmentReport(
        { profile: 'ENTREPRENEUR_SME', sector: 'DIGITAL_TECH',
          horizon: 'MEDIUM', capitalScale: 'SMALL' },
  ...
  const navItems = [
    ...
    { id: 'operator', label: 'Operator', icon: CitizenIcon },
    { id: 'markets', label: 'Markets', icon: TrendingUp },
    { id: 'economic-reality', label: 'Reality', icon: DollarSign },
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <button 
                    ...
                    className="px-2 py-1 text-[9px] font-bold uppercase bg-intel-cyan/10 text-intel-cyan rounded border border-intel-cyan/20 hover:bg-intel-cyan/20"
                  >
                    Refresh
                  </button>
                  <button 
                    onClick={() => setCleared(true)}
                    className="px-2 py-1 text-[9px] font-bold uppercase bg-intel-red/10 text-intel-red rounded border border-intel-red/20 hover:bg-intel-red/20"
                  >
                    Clear
                  </button>
                </div>
              ...
              {displayedEvents.filter(e => e && e.id).map((evt, idx) => (
                <FeedCard 
                  ...
                  item={{
                    id: evt.id,
                    title: evt.title,
                    summary: (evt as any).summary || (evt as any).description || 'No summary available.',
                    impact: ((evt as any).severity || 1) >= 3 ? 'CRITICAL' : 'HIGH',
                    confidence: 85,
                    time: 'Just now',
                    category: (evt as any).category || 'GENERAL',
                    votes: (evt as any).articleCount || 1
                  }} 
                />
              ))}
            </div>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-intel-cyan to-intel-purple p-[1px]">
                  <div className="w-full h-full rounded-full bg-intel-bg flex items-center justify-center overflow-hidden">
                    <CitizenIcon className="w-5 h-5 text-slate-400" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {quickReport && (
                <div className={`p-4 rounded-xl border ${
                  quickReport.timingVerdict === 'ENTER_NOW'
                    ? 'border-intel-cyan/30 bg-intel-cyan/5'
                    : quickReport.timingVerdict === 'WAIT_FOR_TRIGGER'
                    ? 'border-intel-orange/30 bg-intel-orange/5'
                    : 'border-intel-border/30 bg-black/10'
                }`}>
                  <div className="text-[10px] text-slate-300 font-medium">
                    Investment Climate: {quickReport.timingVerdict.replace(/_/g,' ')}
                  </div>
                  <div className="text-[9px] text-slate-500 mt-1">
                    {quickReport.entryWindow}
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-hide">
                <button
                  ...
                  className={cn(
                    ...
                    selectedEventType === 'all' ? "bg-intel-cyan/20 text-intel-cyan border border-intel-cyan/30" : "bg-white/5 text-slate-500 border border-intel-border"
                  )}
                >
                  ALL
                </button>
                {['protest', 'arrest', 'economic', 'water', 'migration', 'internet', 'political', 'detention', 'infrastructure', 'rights', 'labor'].map(type => (
                  <button
                    ...
                    className={cn(
                      "px-3 py-1 rounded-lg text-[8px] font-mono font-bold uppercase transition-all whitespace-nowrap",
                      selectedEventType === type ? "bg-intel-cyan/20 text-intel-cyan border border-intel-cyan/30" : "bg-white/5 text-slate-500 border border-intel-border"
                    )}
                  >
                    {type}
                  </button>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button 
                ...
                aria-label="Download Intelligence Report"
              ...
              <button 
                ...
                aria-label="Open Data Pipeline and Database"
                className="p-4 rounded-2xl border border-intel-border bg-white/5 text-[10px] font-bold uppercase tracking-widest hover:bg-intel-cyan/10 hover:border-intel-cyan/40 hover:text-intel-cyan transition-all flex flex-col items-center space-y-2"
              >
              ...
              <button 
                onClick={onGoHome} 
                aria-label="Switch Operational Modes"
                className="p-4 rounded-2xl border border-intel-border bg-white/5 text-[10px] font-bold uppercase tracking-widest hover:bg-intel-red/10 hover:border-intel-red/40 hover:text-intel-red transition-all flex flex-col items-center space-y-2"
              >
                <Zap className="w-5 h-5" />
                <span>Modes</span>
              </button>
