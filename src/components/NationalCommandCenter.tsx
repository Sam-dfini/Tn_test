...
const ArcGauge: React.FC<{
  ...
  function arcPath(
    ...
    const s1 = polar(startDeg, outerR),
      e1 = polar(endDeg, outerR);
    const s2 = polar(endDeg, innerR),
      e2 = polar(startDeg, innerR);
  ...
  return (
    <div className="relative w-full flex flex-col items-center py-10">
      ...
      >
            ...
            <g key={`segment-${i}`}>
              ...
              <text
                ...
                style={{
                  ...
                  textShadow: sectionActive ? `0 0 15px ${z.color}80` : "none",
                }}
              >
                {z.label}
              </text>
        ...
        <g>
          ...
          <circle
            ...
            r="10"
            fill={status.color}
            style={{ filter: `drop-shadow(0 0 20px ${status.color})` }}
          />
        </g>
      ...
      {/* Status Indicators - Down the Gauge / At the Bottom */}
      <div className="mt-8 text-center space-y-4">
        <div
          className="text-3xl md:text-4xl font-mono font-black tracking-[0.6em] uppercase"
          style={{
            color: status.color,
            textShadow: `0 0 30px ${status.color}60`,
          }}
        >
          {status.label}
        </div>
        <div className="text-[13px] md:text-base font-mono font-bold uppercase tracking-[0.3em] text-slate-400 max-w-[500px] mx-auto leading-relaxed border-t border-white/5 pt-5">
          {status.sub}
        </div>
      </div>
    </div>
...
const PressureRow: React.FC<{
  ...
  return (
    <div className="flex items-center gap-3 py-3 border-b border-white/5 last:border-0 hover:bg-white/[0.02] px-2 rounded-lg transition-colors group">
      <div className="w-10 text-[9px] font-mono text-slate-600 shrink-0 font-bold">
        {id}
      </div>
...
const DomainTile: React.FC<{
  ...
  >
    {alert && (
      <div
        className="absolute top-2 right-2 md:top-3 md:right-3 w-2 h-2 md:w-2.5 md:h-2.5 rounded-full animate-pulse shadow-[0_0_12px_rgba(239,68,68,1)]"
    ...
    <div className="p-2 md:p-3 rounded-2xl bg-white/[0.03] border border-white/[0.05] group-hover:bg-white/[0.06] group-hover:border-white/[0.1] transition-all duration-300 shadow-inner">
      <Icon
        className="w-5 h-5 md:w-8 md:h-8 transition-transform group-hover:rotate-12 group-hover:scale-110"
        style={{ color }}
      />
    </div>
    ...
    <div className="space-y-4 relative pb-10 p-3 md:p-4">
      ...
      <div className="glass rounded-xl border border-intel-border/50 overflow-hidden">
        <div className="flex items-center gap-0 overflow-x-auto no-scrollbar divide-x divide-white/5">
          ...
          <div className="flex items-center gap-2 px-4 py-3 shrink-0">
            ...
            <span
              className="text-[9px] font-mono font-bold uppercase tracking-wider whitespace-nowrap"
                  ...
                  <div className="flex justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <DeltaIcon
                        ...
                        style={{
                          color: t.deltaDir === "up" ? "#ef4444" : "#10b981",
                        }}
                      />
                      <span
                        className={
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="text-[8px] font-mono text-slate-700">
            Actionable operational decision modules
          </div>
        </div>
        ...
        <div className="glass rounded-2xl border border-intel-border/50 overflow-hidden">
          ...
          <div className="p-3">
            {pressureRows.map((row, i) => (
              <PressureRow key={`pressure-${row.label}-${i}`} {...row} />
            ))}
          </div>
        </div>
        ...
        >
          <div className="text-left">
            ...
            <div className="text-[8px] font-mono text-slate-600 uppercase tracking-widest mt-0.5">
              Deep Analysis Mode | Force graph · Evidence chain · Source
              authority · Full simulation engine
            </div>
          </div>
          {analystOpen ? (
                ...
                {[
                  {
                    ...
                    desc: "AI-synthesized daily SITREP",
                    tab: "overview",
                    icon: Radio,
                  },
                  {
                    label: "Key Intelligence Questions",
                    desc: "3 active KIQs — UGTT, Gafsa, EU-TN",
                    tab: "overview",
                    icon: Target,
                  },
                  {
                    label: "Scenario Distribution",
                    desc: "Default: 45% / Social: 30% / IMF: 15%",
                    tab: "strategic",
                    icon: BarChart3,
                  },
                  {
                    label: "Actor Posture Matrix",
                    desc: "Regime · UGTT · Opposition · Youth · IMF",
                    tab: "political",
                    icon: Users,
                  },
                  {
                    label: "Narrative Cluster Map",
                    desc: "Gov Stability vs SNJT Denunciation",
                    tab: "narrative",
                    icon: Brain,
                  },
                  {
                    label: "Live Signal Feed",
                    desc: "Real-time classified intelligence",
                    tab: "events",
                    icon: Zap,
                  },
                ].map((item, i) => {
