...
export const RiskModel: React.FC<RiskModelProps> = ({ variables: initialVariables }) => {
  ...
  const handleVariableChange = (id: string, newValue: number) => {
    setVariables(prev => (prev || []).map(v => {
      const varId = v.id || `${v.code}${v.number}`;
      return varId === id ? { ...v, value: newValue, value_2026: newValue * 100 } : v;
    }));
  };
  ...
  const handleWeightChange = (id: string, newWeight: number) => {
    setVariables(prev => (prev || []).map(v => {
      const varId = v.id || `${v.code}${v.number}`;
      return varId === id ? { ...v, weight: newWeight } : v;
    }));
  };
          ...
          <div className="glass p-6 rounded-2xl border border-intel-border space-y-3">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-2">
                {rriState.threshold_breaches.slice(0, 8).map((breach: any) => (
                  <span key={breach.variable} className="text-[8px] font-mono px-2 py-0.5
              ...
              return (
                <div key={category} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-8">
                    {categoryVars.map((v, index) => {
                      const varId = v.id || `${v.code}${v.number}`;
                      const varValue = v.value ?? (v.value_2026 / 100);
                      return (
                        <div key={`${varId}-${index}`} className="space-y-3 relative">
                          <div className="flex items-center justify-between">
                            <div 
                              className="flex flex-col cursor-help group/var"
                              onMouseEnter={() => setHoveredVarId(varId)}
                                      ...
                                      <div className="space-y-2">
                                        <div className="flex items-start space-x-2">
                                          ...
                                          <div className="space-y-0.5">
                                            ...
                                            <div className="text-[10px] text-slate-300">{v.source || 'TunisiaIntel Core'}</div>
                                          </div>
                                        </div>
                                        ...
                                        <div className="flex items-start space-x-2">
                                          ...
                                          <div className="space-y-0.5">
                                            ...
                                            <div className="text-[10px] text-slate-300">{v.last_updated || new Date().toISOString().split('T')[0]}</div>
                                          </div>
                                        </div>
                            ...
                            <div className="flex items-center space-x-3">
                              <div className="flex flex-col items-end">
                                ...
                                <input 
                                  ...
                                  max="1"
                                  value={v.weight}
                                  onChange={(e) => handleWeightChange(varId, parseFloat(e.target.value))}
                          ...
                          <input 
                            ...
                            step="0.01"
                            value={varValue}
                            onChange={(e) => handleVariableChange(varId, parseFloat(e.target.value))}
                            className="w-full h-1.5 bg-intel-border rounded-full appearance-none cursor-pointer accent-intel-cyan"
                          />
                          <div className="flex justify-between text-[8px] font-mono text-slate-500 uppercase">
                            <span>{(v.direction || (v.invert ? 'negative' : 'positive')) === 'positive' ? 'Low Risk' : 'High Risk'}</span>
                            <span>{(v.direction || (v.invert ? 'negative' : 'positive')) === 'positive' ? 'High Risk' : 'Low Risk'}</span>
                          </div>
                        </div>
                      );
                    })}
