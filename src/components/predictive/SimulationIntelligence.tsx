...
import { runMonteCarlo, simulateScenario, calculatePRev as engineCalculatePRev } from '../../math/rri/engine';
import { RRIVariable } from '../../types/intel';
import { generateStableKey, prepareList, assertKey } from '../../lib/keyUtils';
...
export const SimulationIntelligence: React.FC<{ context?: any, variables: RRIVariable[] }> = ({ context, variables }) => {
  ...
  const simVariables = variables && variables.length > 0 ? variables : DEFAULT_SCENARIO_VARIABLES as any[];

  // Monte Carlo State
  const [mcData, setMcData] = useState(() => runMonteCarlo(simVariables as any));
  
  // Scenario State
  const [scenarioParams, setScenarioParams] = useState<Record<string, number>>(() => {
    const initial: Record<string, number> = {};
    simVariables.forEach(v => {
      initial[v.id || `${v.code}${v.number}`] = v.value || 0;
    });
    return initial;
  });
  ...
  const currentPRev = useMemo(() => {
    if (variables && variables.length > 0) {
      ...
      return calculatePRev(scenarioParams);
    }
          ...
          <div className="intel-card p-6 rounded-2xl border border-intel-border bg-gradient-to-br from-intel-card to-intel-cyan/5">
            ...
            <div className="space-y-4">
              {prepareList([
                ...
                <div key={generateStableKey(stat, i, 'stat')} className="flex items-center justify-between border-b border-white/5 pb-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 intel-card p-6 rounded-2xl border border-intel-border">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center space-x-2">
              {prepareList(SCENARIO_PRESETS).map((preset: any, i: number) => (
                <button 
                  key={generateStableKey(preset, i, 'preset')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
            {prepareList(simVariables).map((v: any, index: number) => {
              const vId = v.id || `${v.code}${v.number}`;
              return (
              <div key={generateStableKey(v, index, 'sim-var')} className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-mono text-intel-cyan font-bold">{((scenarioParams[vId] || 0) * 100).toFixed(0)}%</span>
                </div>
                <input 
                  type="range" 
                  min="0" 
                  max="1" 
                  step="0.01" 
                  value={scenarioParams[vId] || 0} 
                  onChange={(e) => handleParamChange(vId, parseFloat(e.target.value))}
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="intel-card p-6 rounded-2xl border border-intel-border space-y-6">
            ...
            <div className="space-y-4">
              ...
              {prepareList([1000, 500, 100]).map((speed: any, i: number) => (
                <button 
                  key={generateStableKey(speed, i, 'speed')}
        ...
        <div className="lg:col-span-8 intel-card p-6 rounded-2xl border border-intel-border flex flex-col">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center space-x-4">
              {prepareList(INITIAL_AGENTS).map((a: any, i: number) => (
                <div key={generateStableKey(a, i, 'init-agent')} className="flex items-center space-x-1">
          ...
          <div className="flex-1 h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={agentHistory}>
                ...
                {prepareList(INITIAL_AGENTS).map((a: any, i: number) => (
                  <Line 
                    key={generateStableKey(a, i, 'line')}
                    type="monotone" 
                    dataKey={a.type} 
                    stroke={a.color} 
                    strokeWidth={2} 
          ...
          <div className="mt-6 pt-6 border-t border-white/5 grid grid-cols-4 gap-4">
            {prepareList([
              { id: 'POLICE_BRUTALITY', label: 'POLICE BRUTALITY', color: 'intel-red' },
              { id: 'UGTT_STRIKE', label: 'UGTT STRIKE', color: 'intel-cyan' },
              { id: 'IMF_REJECTION', label: 'IMF REJECTION', color: 'intel-orange' },
              { id: 'REMITTANCE_SURGE', label: 'REMITTANCE SURGE', color: 'intel-green' }
            ]).map((event: any, i: number) => (
              <button 
                key={generateStableKey(event, i, 'shock')}
        ...
        {aiResponses.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-9 grid grid-cols-1 md:grid-cols-2 gap-4">
              {prepareList(aiResponses).map((response: any, i: number) => {
                const persona = ANALYST_PERSONAS.find(p => p.id === response.id)!;
                return (
                  <motion.div 
                    key={generateStableKey(response, i, 'ai-analyst')}
