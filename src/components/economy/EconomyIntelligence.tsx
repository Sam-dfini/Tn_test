...
const economicAlerts = [
  ...
  { code: 'ECON-BCT-04', title: 'Forex Reserves Rebound to 112 Days', impact: 'GOOD' },
...
const criticalGoodsAlerts = [
  { item: 'Butane Gas', alert: 'Severe shortage across the entire North West region; distribution networks disrupted.', level: 'CRITICAL' },
...
type EconomyCategory = 'MACRO' | 'REMITTANCES' | 'SECTOR' | 'MARKET' | 'STARTUP' | 'SOCIAL' | 'REGIONAL' | 'BUSINESS' | 'SHORTAGES';

export const EconomyIntelligence: React.FC = () => {
  const { fullData: data, seiResult } = useRiskMetrics();
  const { articles } = useRSS();

  const { shortages, energyShocks } = React.useMemo(() => {
    if (articles.length === 0) return {
      shortages: [], energyShocks: [], compoundScores: {}
    };
    return detectShortagesInArticles(articles, 72);
  }, [articles]);

  const seasonalForecast = React.useMemo(() => getSeasonalForecast(), []);
  ...
  const [activeCategory, setActiveCategory] = useState<EconomyCategory>('MACRO');
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={[
                  ...
                  <Bar dataKey="val" radius={[4, 4, 0, 0]}>
                    { prepareList([37.3, 49.7, 57.0, 44.3]).map((entry: any, index: number) => (
        ...
        <div className="glass p-4 md:p-8 rounded-xl md:rounded-3xl border border-intel-border space-y-6">
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={prepareList(remittanceDistribution)}
                  innerRadius={60}
                  outerRadius={80}
                  paddingAngle={5}
                  dataKey="value"
                >
                  {prepareList(remittanceDistribution).map((entry: any, index: number) => (
        ...
        {isLoading && macroIndicators.length === 0 ? (
          ...
          prepareList((macroIndicators.length > 0 ? macroIndicators : [
            ...
            { label: 'Phosphate Output', value: '3.1Mt', trend: '-28 vs 2025', status: 'CRITICAL', desc: 'Phosphate production 2025. Strikes, equipment failure, water shortage. Peak was 8Mt (2010).', source: 'CPG / GCT', history: [4.2, 3.8, 3.5, 3.3, 3.1] }
              ...
              className={`glass p-4 rounded-xl border transition-all duration-300 relative overflow-hidden group flex flex-col justify-between h-52 ${
                ind.status === 'CRITICAL' ? 'border-intel-red/20 hover:border-intel-red/50' :
                ind.status === 'WARNING' ? 'border-intel-orange/20 hover:border-intel-orange/50' :
                'border-intel-green/20 hover:border-intel-green/50'
              }`}
            >
              ...
              {/* Status Glow */}
              {ind.status === 'CRITICAL' && (
                <div className="absolute top-0 right-0 w-20 h-20 bg-intel-red/10 blur-3xl rounded-full -mr-8 -mt-8 animate-pulse"></div>
              )}
                ...
                <div className="flex items-end justify-between">
                  ...
                  {ind.links && (
                    <div className="flex flex-col gap-1 mt-2">
                      {prepareList(ind.links).map((link: any, lIdx: number) => (
                        <button 
                  ...
                  {/* Mini Sparkline */}
                  <div className="h-12 w-full opacity-60 group-hover:opacity-100 transition-opacity mt-2">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={prepareList(ind.history || []).map((v: any, idx: number) => ({ v, idx }))}>
                        <Tooltip 
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={[
                ...
                <Bar dataKey="val" radius={[0, 4, 4, 0]} barSize={12}>
                  { prepareList([9.3, 8.5, 7.2, 6.1, 5.8, 4.2, 3.5, 2.1]).map((entry: any, index: number) => (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={[
                ...
                <Bar dataKey="val" radius={[2, 2, 0, 0]}>
                  { prepareList([850, 1100, 450, 600, 780, 1250]).map((entry: any, index: number) => (
          ...
          <div className="space-y-4">
            ...
            <div className="h-40 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={prepareList(debtBreakdownData)}
                    cx="50%"
                    cy="50%"
                    innerRadius={40}
                    outerRadius={60}
                    ...
                    dataKey="value"
                  >
                    {prepareList(debtBreakdownData).map((entry: any, index: number) => (
            ...
            <div className="grid grid-cols-1 gap-2">
              {prepareList(debtBreakdownData).map((item: any, i: number) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sortedSectorData} layout="vertical" barGap={2}>
                ...
                <Bar dataKey="growth" name="Growth Rate %" radius={[0, 2, 2, 0]} barSize={10}>
                  { prepareList(sortedSectorData).map((entry: any, index: number) => (
        ...
        <div className="glass p-6 md:p-8 rounded-3xl border border-intel-border space-y-6">
          ...
          <div className="space-y-6">
            {prepareList(sectorData).map((sector: any, i: number) => {
  ...
  const renderMarketIntelligence = () => (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {prepareList(marketIndicators).map((ind: any, i: number) => (
          <motion.div 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass p-6 md:p-8 rounded-3xl border border-intel-border space-y-6">
          ...
          <div className="h-[800px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={foodBasketByGovernorate} layout="vertical" margin={{ left: 10, right: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" horizontal={false} />
                <XAxis type="number" axisLine={false} tickLine={false} tick={{ fill: '#475569', fontSize: 8, fontFamily: 'monospace' }} />
                <YAxis 
                  dataKey="name" 
                  ...
                  axisLine={false} 
                  tickLine={false} 
                  tick={{ fill: '#cbd5e1', fontSize: 10, fontFamily: 'monospace', fontWeight: 'bold' }} 
                  width={120}
                  interval={0}
                />
                <Tooltip contentStyle={{ backgroundColor: '#0a0a0a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '10px' }} />
              ...
              <PieChart>
                ...
                >
                  {prepareList([
                    ...
                    <Cell key={generateStableKey(entry, index, 'market-comp')} fill={entry.color} />
                  ))}
                </Pie>
                <Tooltip contentStyle={{ backgroundColor: '#0a0a0a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '10px' }} />
                <Legend verticalAlign="bottom" align="center" iconType="circle" wrapperStyle={{ fontSize: '8px', fontFamily: 'monospace', textTransform: 'uppercase', paddingTop: '20px' }} />
          ...
          <div className="h-96 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={povertyByGovernorate} layout="vertical">
                ...
                <Line type="monotone" dataKey="governance" name="Governance Score" stroke="#00f2ff" strokeWidth={2} dot={{ r: 3, fill: '#00f2ff' }} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
  ...
  const renderPharmacyIntelligence = () => (
    <div className="space-y-8 animate-in fade-in duration-500">
          ...
          <div className="glass p-6 rounded-2xl border border-intel-border space-y-6">
            ...
            <div className="h-48 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={prepareList(importDependencyData)}
                    cx="50%"
                    cy="50%"
                    innerRadius={40}
                    outerRadius={60}
                    ...
                    dataKey="value"
                  >
                    {prepareList(importDependencyData).map((entry: any, index: number) => (
            ...
            <div className="space-y-2">
              {prepareList(importDependencyData).map((item: any, i: number) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {prepareList(essentialMedsStatus).map((med: any, i: number) => (
  ...
  const renderStartupIntelligence = () => (
    <div className="space-y-8 animate-in fade-in duration-500">
      {/* Ecosystem Metrics */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {prepareList(startupEcosystemMetrics).map((ind: any, i: number) => (
          <motion.div 
        ...
        <div className="glass p-6 md:p-8 rounded-3xl border border-intel-border space-y-6">
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={prepareList(startupSectors)}
                  innerRadius={60}
                  outerRadius={80}
                  paddingAngle={5}
                  dataKey="value"
                >
                  {prepareList(startupSectors).map((entry: any, index: number) => (
      ...
      <div className="glass rounded-2xl border border-intel-border overflow-hidden overflow-x-auto scrollbar-hide">
        ...
        <table className="w-full text-left border-collapse min-w-[800px]">
          ...
          <tbody className="divide-y divide-intel-border/30">
            {prepareList(regionalData).map((peer: any, i: number) => (
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={regionalData}>
                ...
                <Bar dataKey="gdp" radius={[4, 4, 0, 0]}>
                  { prepareList(regionalData).map((entry: any, index: number) => (
  ...
  return (
    <div className="space-y-6 md:space-y-12 animate-in fade-in duration-700 relative">
      ...
      <div className="relative">
        <div className="flex items-center gap-2 border-b border-intel-border/30 pb-4 sticky top-0 bg-black/50 backdrop-blur-md z-50 py-2 overflow-x-auto scrollbar-hide max-w-full">
          ...
          {activeCategory === 'SHORTAGES' && (
            <div className="space-y-8">
              ...
              <div className="space-y-10">
                {/* SEI — Shortage Escalation Index */}
                {seiResult && (
                  <div className="space-y-6">
                    <div className="flex items-center justify-between">
                      <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Shortage Escalation Index (SEI)</div>
                      <div className="flex items-center space-x-2">
                        <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Max SEI:</span>
                        <span className={`text-xl font-bold font-mono ${
                          seiResult.maxSEI > 0.7 ? 'text-intel-red' : 
                          seiResult.maxSEI > 0.4 ? 'text-intel-orange' : 'text-intel-cyan'
                        }`}>
                          {seiResult.maxSEI.toFixed(2)}
                        </span>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      {prepareList(seiResult.commodities).map((c: any, i: number) => {
                        ...
                        return (
                          <div key={generateStableKey(c, i, 'sei-commodity')} className="glass p-5 rounded-2xl border border-intel-border/30 space-y-4 relative overflow-hidden group">
                            ...
                            <div className="absolute top-0 left-0 w-full h-1 bg-white/5">
                              <div 
                                className={`h-full transition-all duration-1000 ${phaseCfg.color.replace('text-', 'bg-')}`}
                                style={{ width: `${(c.phase / 6) * 100}%` }}
                              />
                            </div>
                            ...
                            <div className="space-y-1">
                              <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">{phaseCfg.label}</div>
                            ...
                            <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5">
                              <div className="space-y-0.5">
                                <div className="text-[8px] font-mono text-slate-500 uppercase">SEI Score</div>
                                <div className={`text-xs font-bold font-mono ${phaseCfg.color}`}>{c.sei.toFixed(2)}</div>
                              </div>
                            ...
                            {c.blackMarketDetected && (
                              <div className="flex items-center space-x-2 text-[9px] font-mono font-bold text-intel-red animate-pulse">
                                <AlertTriangle className="w-3 h-3" />
                                <span>BLACK MARKET DETECTED</span>
                              </div>
                            )}
                          </div>
                        );
                    ...
                    {seiResult.angerWindowAlert && (
                      <div className="p-4 rounded-xl border border-intel-red/30 bg-intel-red/5 flex items-start space-x-4">
                        <Skull className="w-5 h-5 text-intel-red shrink-0 mt-0.5" />
                        <div className="space-y-1">
                          <div className="text-[11px] font-bold text-intel-red uppercase tracking-widest">Compound Trigger: Anger Ignition Window Open</div>
                      ...
                      {prepareList(energyShocks).map((shock: any, i: number) => (
                        <div key={generateStableKey(shock, i, 'energy-shock')}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="px-2 py-1 bg-black/40 rounded border border-white/10 text-[10px] font-mono text-slate-400">
                              via {shock.source}
                            </div>
                          </div>
                          ...
                          <div className="flex flex-wrap items-center gap-4 text-[10px] font-mono">
                            <div className="flex items-center space-x-2">
                              <Clock className="w-3 h-3 text-intel-orange" />
                              <span className="text-intel-orange font-bold uppercase">
                                Tunisia impact: ~{shock.estimatedTunisiaImpactDays} days
                              </span>
                            </div>
                            <div className="text-slate-600">|</div>
                            <div className="text-slate-400">
                              Watch: BCT reserves · STEG tariffs · CGC announcements
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                {/* Active shortages from RSS */}
                <div className="space-y-4">
                  <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Active Shortages (72h window)</div>
                  {shortages.length === 0 ? (
                    <div className="text-sm font-mono text-slate-600 py-12 text-center border border-dashed border-intel-border/30 rounded-2xl bg-white/5">
                      No shortage signals detected in the last 72 hours.
                      <br />
                      <span className="text-[10px] opacity-50">RSS feeds are continuously monitored for supply chain disruptions.</span>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {prepareList(shortages).map((s: any, i: number) => {
                        const severityColor =
                          s.severity >= 4 ? 'text-intel-red border-intel-red/30 bg-intel-red/10' :
                        ...
                        return (
                          <div key={generateStableKey(s, i, 'shortage-item')}
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              <div className="px-2 py-0.5 rounded bg-black/20 text-[9px] font-bold">
                                SEV {s.severity}/5
                              </div>
                            </div>
                            {s.governorate && (
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-current/10">
                              <div className="text-[10px] font-mono">
                                {s.articleCount} reports
                              </div>
                              <div className="text-[10px] font-mono font-bold">
                                RRI +{s.rriNudge.toFixed(3)}
                              </div>
                            </div>
                          </div>
                        );
                ...
                {/* Seasonal forecast */}
                <div className="space-y-4">
                  <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">Seasonal Risk Forecast</div>
                  {seasonalForecast.length === 0 ? (
                    <div className="text-sm font-mono text-slate-600 py-6 text-center">No elevated seasonal risks in next 6 weeks.</div>
                  ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {prepareList(seasonalForecast.slice(0, 6)).map((f: any, i: number) => (
                        <div key={generateStableKey(f, i, 'seasonal-forecast')} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                          ...
                          <div className="space-y-1">
                            <div className={`text-xs font-bold uppercase tracking-widest ${
                              f.priority === 'high' ? 'text-intel-red' : 'text-intel-orange'
                            }`}>{f.type}</div>
                            <div className="text-[10px] text-slate-400 leading-relaxed">
                              {f.warning}
                            </div>
                          </div>
                          <div className="text-[10px] font-mono text-slate-500 font-bold shrink-0 ml-4 px-3 py-1 bg-black/20 rounded-lg">
                            {f.timeframe}
                          </div>
                        </div>
                      ))}
                ...
                {/* New RRI variables status */}
                <div className="space-y-4">
                  <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest">
                    Basic Needs & Energy Security Indices
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {prepareList([
                      { id: 'B22', label: 'LPG Availability', warn: true },
                      { id: 'B23', label: 'Electricity Reliability', warn: true },
                      { id: 'B24', label: 'Food Staples', warn: true },
                      { id: 'B25', label: 'Fuel Pumps', warn: false },
                      { id: 'H01', label: 'Energy Import Cost', warn: true },
                      { id: 'H03', label: 'CGC Subsidy Fund', warn: true },
                      { id: 'H04', label: 'Global Energy Shock', warn: energyShocks.length > 0 },
                      { id: 'H05', label: 'Seasonal Demand', warn: false },
                    ]).map((v: any) => (
                      <div key={v.id}
                        className="flex items-center justify-between p-3 rounded-xl bg-black/40 border border-intel-border/30 hover:border-intel-cyan/30 transition-colors">
                        <div className="space-y-0.5">
                          <div className="text-[8px] font-mono text-slate-500 uppercase">{v.label}</div>
                          <div className={`text-xs font-bold ${v.warn ? 'text-intel-orange' : 'text-intel-cyan'}`}>{v.id}</div>
                        </div>
                        <div className={`w-1.5 h-1.5 rounded-full ${v.warn ? 'bg-intel-orange animate-pulse' : 'bg-intel-cyan'}`} />
                      </div>
                    ))}
                  </div>
                </div>
