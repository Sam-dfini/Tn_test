  ...
  return (
    <div className="bg-[#0a0c10] border border-white/5 rounded-2xl overflow-hidden">
      ...
      <div className="grid grid-cols-3 md:grid-cols-6 divide-x divide-white/5">
        {prepareList(kpis).map((kpi: any, i: number) => {
          const Icon = kpi.icon;
          return (
            <div key={generateStableKey(kpi.label, i, 'kpi')} className="px-4 py-3 space-y-1 hover:bg-white/[0.02] transition-colors">
      ...
      <div className="border-t border-white/5 flex items-center gap-0 overflow-x-auto no-scrollbar">
        ...
        {prepareList(TOP_ALERTS).map((a: any, i: number) => (
          <div key={generateStableKey(a, i, 'alert')} className={`flex items-center gap-2 px-4 py-2 border-r border-white/5 shrink-0 ${i === 0 ? 'animate-pulse' : ''}`}>
...
const PersonaSelector: React.FC<{
  ...
  <div className="space-y-2">
    ...
    <div className="flex items-center gap-2 flex-wrap">
      {prepareList(PERSONAS).map((p: any, i: number) => {
        const Icon = p.icon;
        const isActive = selected === p.id;
        return (
          <button
            key={generateStableKey(p, i, 'persona')}
...
const SectorSnapshot: React.FC = () => (
  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2">
    {prepareList(SECTOR_SCORES).map((s: any, i: number) => (
      <div key={generateStableKey(s, i, 'sector')} className="bg-[#0a0c10] border border-white/5 rounded-xl p-3 hover:border-white/10 transition-all">
