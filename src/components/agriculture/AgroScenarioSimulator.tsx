        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="space-y-3 pt-4 border-t border-white/5">
            ...
            <div className="grid grid-cols-1 gap-2">
              {prepareList(SCENARIO_PRESETS).map((s: any, i: number) => (
                <button 
                  key={generateStableKey(s, i, 'scenario')}
                  onClick={() => {
            ...
            <div className="space-y-4">
               <h4 className="text-[10px] font-mono text-slate-500 uppercase tracking-widest flex items-center">
                ...
                <div className="space-y-4 relative pl-4 border-l border-white/5">
                  {prepareList(causalityChain).map((step: any, i: number) => (
                    <motion.div 
                     key={generateStableKey(step, i, 'step')}
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        <span className="text-[11px] font-medium text-white/90 uppercase tracking-tight">
                          {typeof step === 'object' ? (step.value || step.id) : step}
                        </span>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={[
                    ...
                    <Bar dataKey="value" radius={[2, 2, 0, 0]}>
                      { prepareList([0, 1, 2]).map((v: any, i: number) => <Cell key={generateStableKey(v, i, 'cell')} fill={ i === 0 ? '#10b981' : i === 1 ? '#f59e0b' : '#3b82f6'} />) }
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
