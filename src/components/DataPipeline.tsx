...
export const DataPipeline: React.FC = () => {
  ...
  const handleReject = (fieldId: string) => {
    setRejectedFields(prev => [...prev, fieldId]);
    setReviewQueue(prev => prev.filter(f => f.id !== fieldId));
  };
                    ...
                    <AnimatePresence mode="popLayout">
                      {reviewQueue.map((field) => (
                        <motion.div 
                          ...
                          exit={{ opacity: 0, x: -20 }}
                          className="p-5 bg-slate-900/60 border border-white/10 rounded-2xl group hover:border-intel-cyan/40 transition-all"
                        >
                            ...
                            <div className="flex-1 space-y-4">
                              <div className="flex items-center justify-between">
                                <div className="space-y-1">
                                  ...
                                  <div className="text-lg font-bold text-white">{FIELD_MAP[field.field]?.label || field.field}</div>
                                </div>
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                <div className="p-3 bg-black/40 rounded-xl border border-white/5">
                                  <div className="text-[9px] font-mono text-slate-500 uppercase mb-1">Extracted Value</div>
