...
import { IdeologicalCycle } from '../social/IdeologicalCycle';
import { useRiskMetrics } from '../../hooks/usePipelineDomains';
import { CycleAnalysisTab } from '../social/CycleAnalysisTab';
import { CivilizationalEngine } from '../social/CivilizationalEngine';
import {
  TrendingDown, AlertTriangle, RotateCcw,
  Zap, Globe, Users, Shield, Brain,
  ChevronRight, Info, BookOpen, Activity
} from 'lucide-react';
...
export const CivilizationalAnalysis: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dalio' | 'haupt' | 'freedom' | 'convergence' | 'taxation' | 'ideology' | 'cycle' | 'engine'>('dalio');
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in duration-700 relative">
      ...
      <ModuleHeader 
        title="Civilizational Analysis"
        subtitle="Cross-dimensional mapping of historical cycles and theoretical frameworks onto live data"
        icon={RotateCcw}
        nodeId="CIV-NODE-12"
      />
      ...
      {/* Sub-Tabs Navigation */}
      <div className="flex items-center space-x-1 bg-white/5 p-1 rounded-xl border border-white/10 w-fit relative z-20 flex-wrap">
        {[
          { id: 'dalio', label: 'Regime Aging Curve', icon: TrendingDown },
          { id: 'haupt', label: 'Civilization Phase', icon: RotateCcw },
          { id: 'freedom', label: 'Freedom Cycle', icon: Users },
          { id: 'convergence', label: 'Convergence', icon: AlertTriangle },
          { id: 'taxation', label: 'Fiscal Doctrine', icon: Zap },
          { id: 'ideology', label: 'Ideological Cycle', icon: Brain },
          { id: 'cycle', label: 'Cycle Analysis', icon: RotateCcw },
          { id: 'engine', label: 'Civilizational Engine', icon: Activity },
        ].map((tab) => (
          <button
            key={tab.id}
      ...
      {/* ============================================================
          VIZ 5 — FISCAL DOCTRINE / TAXATION PRESSURE
      ============================================================ */}
      {activeTab === 'taxation' && (() => {
        const informalPct = data.economy?.informal_economy_pct ?? 47;
        const trustState = (data.social?.happiness_index ?? 4.2) / 10;
        const formalTaxBurden = 85;
        const informalTaxBurden = 2;
        const tpt = (formalTaxBurden * (1 - informalPct/100))
                  - (informalTaxBurden * (informalPct/100))
                  + (formalTaxBurden - informalTaxBurden) * 0.3;
        const tptNorm = Math.min(10, tpt / 8);
        const alpha = 0.18;
        const deltaRtax = alpha * tptNorm * (1 - trustState);

        const doctrineAxes = [
          { label: 'State Control',    value: 82, color: '#ff453a', desc: 'Strong executive intervention in economy and politics' },
          { label: 'Social Spending',  value: 74, color: '#ff9f0a', desc: 'Subsidies, public employment, social transfers' },
          { label: 'Market Liberalism',value: 28, color: '#30d158', desc: 'Weak private sector, constrained competition' },
          { label: 'Fiscal Extraction',value: 91, color: '#bf5af2', desc: 'Formal sector over-taxed — informal sector escapes' },
        ];

        const sectors = [
          { name: 'SMEs / Formal Traders',    burden: 88, escape: 5,  angerLevel: 92, revolt: 'HIGH' },
          { name: 'Transport (Taxis, Trucks)', burden: 72, escape: 25, angerLevel: 78, revolt: 'HIGH' },
          { name: 'Salaried Workers (CNSS)',   burden: 65, escape: 0,  angerLevel: 65, revolt: 'MEDIUM' },
          { name: 'Large Corporations',        burden: 35, escape: 55, angerLevel: 30, revolt: 'LOW' },
          { name: 'Informal Economy',          burden: 2,  escape: 98, angerLevel: 20, revolt: 'LOW' },
          { name: 'Rent-Seeking Networks',     burden: 8,  escape: 88, angerLevel: 15, revolt: 'VERY LOW' },
        ];

        return (
          <div className="space-y-6">
            <p className="text-slate-400 text-sm leading-relaxed max-w-3xl">
              Tunisia's tax system is asymmetric extraction. The formal economy is over-burdened
              while informal networks escape entirely. Combined with low trust in state services,
              this generates concentrated anger in the commercial middle class — the
              "الاقتصاد الريعي" narrative resonates because it is empirically correct.
            </p>

            <div className="glass p-6 rounded-2xl border border-intel-border/50 space-y-4">
              <div className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">Economic Doctrine — Four-Axis Framework</div>
              <div className="space-y-3">
                {doctrineAxes.map(axis => (
                  <div key={axis.label} className="space-y-1.5">
                    ...
                    <div className="relative h-3 bg-slate-800 rounded-full overflow-hidden">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${axis.value}%` }} transition={{ duration: 1 }}
                        className="h-full rounded-full" style={{ backgroundColor: axis.color }} />
                    </div>
                    <div className="text-[8px] text-slate-600">{axis.desc}</div>
                  </div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <EquationCard 
                number="19"
                title="Taxation Pressure Index"
                source="paper"
                latex="TP(t) = \tau_{formal} \cdot W_{formal} - \tau_{informal} \cdot W_{informal} + \epsilon_{gap}"
                currentOutput={`${tptNorm.toFixed(2)} / 10`}
                description="Quantifies the asymmetric fiscal burden between the formal and informal sectors."
              />
              <EquationCard 
                number="20"
                title="Fiscal Risk Contribution"
                source="paper"
                latex="\Delta R_{tax} = \alpha \cdot TP(t) \cdot (1 - Trust_{state})"
                currentOutput={`+${deltaRtax.toFixed(4)}`}
                description="Measures the direct contribution of fiscal pressure to the overall revolutionary risk score."
              />
            </div>

            <div className="glass p-6 rounded-2xl border border-intel-border/50 space-y-3">
              <div className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">Who Revolts First — Sector Tax Pressure vs Anger</div>
              {sectors.map(s => {
                const ac = s.angerLevel >= 80 ? '#ff453a' : s.angerLevel >= 60 ? '#ff9f0a' : '#30d158';
                return (
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[{ label: 'Tax burden', val: s.burden, color: '#ff453a' }, { label: 'Tax escape', val: s.escape, color: '#00f2ff' }, { label: 'Anger', val: s.angerLevel, color: ac }].map(m => (
                        <div key={m.label} className="space-y-0.5">
                          ...
                          <div className="text-[8px] font-mono" style={{ color: m.color }}>{m.val}%</div>
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      {activeTab === 'cycle' && <CycleAnalysisTab />}

      {activeTab === 'engine' && <CivilizationalEngine />}
      ...
      <div className="p-5 rounded-2xl bg-intel-card/30
        ...
        <div className="space-y-1">
          ...
          <div className="text-[10px] text-slate-500 space-y-1">
            ...
            <div>TUNISIAINTEL Extensions (2026). EQ.15-23. rri_variables.json.</div>
          </div>
        </div>
      </div>
    </div>
...
const RegimeAgingCurve: React.FC<{ rriState: any }> = ({ rriState }) => {
  ...
  const curveData = [
    ...
    { year: currentYear, power: 18, label: 'NOW', event: `R(t)=${rriState.rri.toFixed(2)} P_rev=${(rriState.p_rev*100).toFixed(0)}%` },
    { year: (parseInt(currentYear) + 1).toString(), power: 14, label: '', event: 'Projected — debt crisis trigger' },
    { year: (parseInt(currentYear) + 3).toString(), power: 8, label: '?', event: 'New Order or collapse' },
  ];

  const CustomTooltip = ({ active, payload, label }: any) => {
    if (!active || !payload?.[0]) return null;
    const d = curveData.find(x => x.year === label);
    const isNow = label === currentYear || label === 'NOW';
    return (
      <div className="bg-[#0a0f1a] border border-intel-cyan/30
        rounded-lg p-3 text-[10px] font-mono max-w-[220px] shadow-2xl">
        ...
        <div className="pt-2 border-t border-white/5 space-y-1">
          <div className="flex justify-between">
            ...
            <span className="text-intel-cyan">{payload[0].value}%</span>
          </div>
          {isNow && (
            <div className="flex justify-between">
              <span className="text-slate-500">Risk Prob (P_rev):</span>
              <span className="text-intel-red">{(rriState.p_rev * 100).toFixed(1)}%</span>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="glass p-6 rounded-2xl border border-intel-border
      ...
      <div className="flex items-center justify-between">
        <div className="text-[9px] font-mono text-slate-500 uppercase
          tracking-widest">Tunisia Regime Power Arc 1956-{parseInt(currentYear) + 3}</div>
      ...
      <ResponsiveContainer width="100%" height={320}>
        <AreaChart data={curveData}
          ...
          <ReferenceLine x="2021" stroke="#ff453a"
            strokeDasharray="4 4" strokeOpacity={0.5} />
          <ReferenceLine x={currentYear} stroke="#ff453a"
          ...
          <ReferenceDot x="2011" y={45} r={4}
            fill="#ff9f0a" stroke="none" />
          <ReferenceDot x={currentYear} y={18} r={6}
...
const CivilizationPhaseWheel: React.FC<{
  ...
  const phases = [
    ...
    },
    {
      ...
      tunisiaEvidence: `${currentYear}-present. Decree 54 (${data.social.decree54_charged} charged). All institutions captured. P_rev=${(rriState.p_rev*100).toFixed(1)}%.`,
      rriVars: ['L126 (Propaganda)', 'D44 (Press freedom)', 'D50 (Institutional trust)'],
      riskImpact: `${(rriState.p_rev * 100).toFixed(1)}%`,
    },
  ];
          ...
          <AnimatePresence>
            {hoveredPhase !== null && (
              <motion.div
                ...
                className="absolute pointer-events-none z-50 bg-[#0a0f1a] border border-intel-cyan/30 rounded-lg p-3 text-[10px] font-mono shadow-2xl min-w-[150px]"
                style={{
                  left: mousePos.x + 20,
                  top: mousePos.y + 20,
                }}
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="font-bold" style={{ color: phases[hoveredPhase].color }}>
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-intel-red font-bold">{phases[hoveredPhase].riskImpact}</span>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            >
              ...
              <div className="space-y-1">
                ...
                <div className="flex flex-wrap gap-1">
                  {phase.rriVars.map((v, index) => (
                    <span key={`${v}-${index}`} className="text-[8px] font-mono
              ...
              <div className="pt-3 border-t border-white/5 flex items-center justify-between">
                <div className="text-[9px] font-mono text-slate-500 uppercase">Risk Contribution</div>
                <div className="text-sm font-bold font-mono" style={{ color: phase.color }}>
                  {phase.riskImpact}
                </div>
              </div>
            </motion.div>
...
const FreedomCycleDiagram: React.FC<{
  ...
  const nodes = [
    ...
    { id: 'faith', label: 'Faith', angle: 310, color: '#30d158',
    ...
    { id: 'courage', label: 'Courage', angle: 350, color: '#30d158',
      ...
      riskProb: '25%',
      active: false, emerging: true },
    { id: 'liberty', label: 'Liberty', angle: 30, color: '#0a84ff',
    ...
    { id: 'apathy', label: 'Apathy', angle: 190, color: '#ff453a',
      ...
      riskProb: '75%',
      active: true },
    { id: 'dependence', label: 'Dependence', angle: 230, color: '#8B0000',
        ...
        <div className="flex justify-center relative">
          ...
          <AnimatePresence>
            {hoveredNode !== null && (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="font-bold" style={{ color: nodes.find(n => n.id === hoveredNode)?.color }}>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between">
                    ...
                    <span className="text-intel-red font-bold">{nodes.find(n => n.id === hoveredNode)?.riskProb}</span>
                  </div>
                  <div className="text-slate-400 mt-1 leading-tight">
