...
const healthcareFundsData = [
  { name: 'CNAM', subscribers: '3.8M', budget: 3.2, deficiency: 1.1, risk: 'CRITICAL', color: '#0ea5e9', rriImpact: 'High' },
  { name: 'CNSS', subscribers: '2.4M', budget: 4.5, deficiency: 0.8, risk: 'HIGH', color: '#f59e0b', rriImpact: 'Medium' },
  { name: 'CNRPS', subscribers: '0.9M', budget: 2.8, deficiency: 1.4, risk: 'CRITICAL', color: '#ef4444', rriImpact: 'Critical' }
];
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
      ...
      <ModuleHeader 
        ...
        subtitle="Real-time monitoring of social friction, psychological stress, and migration flows"
        icon={Users}
        nodeId="SOCIAL-NODE-05"
      />
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 sticky top-0 z-50 bg-black/40 backdrop-blur-xl p-3 md:p-4 rounded-xl border border-white/5 shadow-2xl">
        <div className="flex items-center space-x-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar max-w-full">
          {prepareList(categories).map((cat: any, i: number) => (
            <button
              key={generateStableKey(cat, i, 'cat')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {prepareList(socialIndicators.filter(ind => ['Happiness Index', 'Youth Rage Index', 'Street Signal S(t)', 'Social Cohesion'].includes(ind.label))).map((ind: any, i: number) => (
            <motion.div 
              key={generateStableKey(ind, i, 'ind')} 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="h-10 w-full mt-2 opacity-30 group-hover:opacity-60 transition-opacity overflow-hidden">
                <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
                  <LineChart data={prepareList(ind.history).map((v: any, idx: number) => ({ v, idx, id: generateStableKey(v, idx, 'v') }))}>
                    <Line 
                      key={generateStableKey(ind.label, i, 'line')}
                      type="monotone" 
                      dataKey="v" 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          ]).map((dossier: any, i: number) => (
            <div key={generateStableKey(dossier, i, 'dossier')} className="glass p-4 md:p-6 rounded-xl md:rounded-2xl border border-intel-border space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {prepareList(socialIndicators.filter(ind => ['Population Pressure'].includes(ind.label))).map((ind: any, i: number) => (
            <motion.div 
              key={generateStableKey(ind, i, 'pop-pres')} 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.05 }}
              whileHover={{ scale: 1.02, backgroundColor: 'rgba(255, 255, 255, 0.05)' }}
            ...
            >
              ...
              <div className="h-10 w-full mt-2 opacity-30 group-hover:opacity-60 transition-opacity">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={prepareList(ind.history).map((v: any, idx: number) => ({ v, idx, id: generateStableKey(v, idx, 'v-pop') }))}>
                    <Line 
                      type="monotone" 
                      dataKey="v" 
                      stroke={ind.status === 'CRITICAL' ? '#ef4444' : '#f97316'} 
          ...
          <div className="glass p-4 md:p-6 rounded-xl md:rounded-2xl border border-intel-border space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {prepareList(familyDynamicsData).map((stat: any, i: number) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {prepareList(socialIndicators.filter(ind => ['Suicide Rate', 'Mental Health Stress', 'Chronic Disease %'].includes(ind.label))).map((ind: any, i: number) => (
            <motion.div 
              key={generateStableKey(ind, i, 'ind-h')} 
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: i * 0.05 }}
              whileHover={{ scale: 1.02, backgroundColor: 'rgba(255, 255, 255, 0.05)' }}
            ...
            >
              ...
              <div className="h-10 w-full mt-2 opacity-30 group-hover:opacity-60 transition-opacity">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={prepareList(ind.history).map((v: any, idx: number) => ({ v, idx, id: generateStableKey(v, idx, 'v-h') }))}>
                    <Line 
                      type="monotone" 
                      dataKey="v" 
                      stroke={ind.status === 'CRITICAL' ? '#ef4444' : '#f97316'} 
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chronicDiseaseBreakdown} layout="vertical">
                  ...
                  <Bar dataKey="value" radius={[0, 4, 4, 0]} barSize={20}>
                    {prepareList(chronicDiseaseBreakdown).map((entry: any, i: number) => (
                      <Cell key={generateStableKey(entry, i, 'cell')} fill={entry.color} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            ...
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={healthcareFundsData}>
                  ...
                  <YAxis axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10, fontFamily: 'monospace' }} tickFormatter={(val) => `${val}B`} />
                  <Tooltip 
                    cursor={{ fill: 'rgba(255,255,255,0.05)' }}
                    contentStyle={{ backgroundColor: '#0a0a0a', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '10px' }}
                    formatter={(val: number) => [`${val}B TND`, '']}
                  />
                ...
                <div key={generateStableKey(fund, i, 'fund')} className="p-4 rounded-xl bg-white/5 border border-intel-border space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="text-sm font-bold text-intel-red">{fund.deficiency}B TND</div>
                    </div>
                    <div className="space-y-1">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={drugAddictionAgeData}>
                  ...
                  <Bar dataKey="percentage" radius={[4, 4, 0, 0]} barSize={40}>
                    {prepareList(drugAddictionAgeData).map((entry: any, i: number) => (
                      <Cell key={generateStableKey(entry, i, 'age-cell')} fill={entry.color} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
        ...
        {/* Rehab Metrics Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {prepareList(rehabMetrics).map((metric: any, i: number) => (
            <div key={generateStableKey(metric, i, 'rehab')} className="glass p-4 md:p-6 rounded-xl md:rounded-2xl border border-intel-border space-y-3">
              <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between border-b border-intel-border/30 pb-4">
              ...
              <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
                {prepareList([
                  ...
                  <button
                    key={generateStableKey(t, i, 'mig-tab')}
                          ...
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={[
                              ...
                              <Bar dataKey="val" radius={[4, 4, 0, 0]}>
                                {prepareList([45, 52, 48, 68, 78, 82, 74, 71]).map((v: any, idx: number) => (
                                  <Cell key={generateStableKey(v, idx, 'bar-cell')} fill={v > 70 ? '#ef4444' : v > 50 ? '#f97316' : '#00f2ff'} />
                                ))}
                              </Bar>
                            </BarChart>
                          </ResponsiveContainer>
                            ...
                            <div className="space-y-2">
                              ...
                              <ul className="space-y-3">
                                {prepareList([
                                  ...
                                  { label: 'Readmission Agreement Friction' },
                                  { label: 'Brain Drain Mitigation Lack' },
                                  { label: 'Transit Migrant Legal Status' }
                                ]).map((item: any, i: number) => (
                                  <li key={generateStableKey(item, i, 'policy')} className="flex items-center space-x-2 text-[10px] text-slate-400">
