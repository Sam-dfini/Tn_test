...
export const RadicalisationIntelligence: React.FC = () => {
  const { rriState, rpiProfile } = useRiskMetrics();
        ...
        >
            ...
            <div className="space-y-4">
              ...
              <div className="glass p-6 rounded-2xl border border-intel-border/50 space-y-4">
                ...
                <div className="space-y-3">
                  {prepareList([0, 1, 2, 3, 4, 5] as EscalationLevel[]).map((lvl: any, idx: number) => {
                    const cfg = LEVEL_CONFIG[lvl as EscalationLevel];
                    const isCurrent = lvl === profile.escalationLevel;
                    const isPassed = lvl < profile.escalationLevel;
                    const isCritical = lvl >= 4;

                    return (
                      <div key={generateStableKey(lvl, idx, 'level')}
          ...
          {activeSection === 'poles' && (
            <div className="space-y-4">
              ...
              {/* Pole bars */}
              <div className="space-y-4">
                {prepareList(Object.entries(POLE_CONFIG) as [NarrativePole, typeof POLE_CONFIG[NarrativePole]][])
                  .filter(([k]: any) => k !== 'MIXED')
                  .map(([pole, cfg]: any, idx: number) => {
                    const score = profile.poleScores[pole as NarrativePole] || 0;
                    const isDominant = profile.dominantPole === pole;
                    return (
                      <div key={generateStableKey(pole, idx, 'pole')}
