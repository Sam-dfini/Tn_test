...
export function parseRSS(xml: string, source: typeof RSS_SOURCES[0]): Omit<Article, 'fetched_at' | 'created_at'>[] {
  ...
  items.forEach(item => {
    try {
      ...
      // Reject articles not relevant to Tunisia before any DB write.
      // This is the primary fix for non-Tunisia items (Denmark train crash,
      // Nigeria airstrikes, etc.) appearing as SYSTEMIC RISK in the feed.
      const geoResult = scoreGeoRelevance(title, cleanContent, source.url, source.geo_weight ?? 0.5);
      if (!geoResult.isRelevant) {
        pipelineDebugger.log('FEED', 'dropped',
          `[GEO-FILTER] Not Tunisia-relevant (score=${geoResult.score}): ${title.slice(0, 60)}`,
          { geoScore: geoResult.score, source: source.name }
        );
        return; // Drop — do not write to Supabase
      }
      // Log geo confidence for monitoring
      pipelineDebugger.log('FEED', 'valid',
        `[GEO-OK] score=${geoResult.score} confidence=${geoResult.confidence}: ${title.slice(0, 50)}`,
        { matchedEntities: geoResult.matchedEntities, source: source.name }
      );
      // ─────────────────────────────────────────────────────────────────────
      ...
      articles.push({
        ...
        geo_relevance_score: geoResult.score,
        confirm_count: 0,
        dispute_count: 0,
        context_count: 0,
        processed: false,
